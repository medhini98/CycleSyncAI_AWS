...
class PlanDetailViewController: UIViewController {
    ...
    init(plan: PlanModel) {
        ...
        // ✅ Step 1: Extract label from full string
        ...
        // ✅ Step 2: Compute using local vars (no 'self')
        let parsedDates = PlanDetailViewController.extractDates(from: label)
        let defaultDate = PlanDetailViewController.chooseDefaultDate(from: parsedDates)

        // ✅ Step 3: Call super
        super.init(nibName: nil, bundle: nil)

        // ✅ Step 4: Now assign to self
        self.dateOptions = parsedDates
        print("🧪 Parsed dateOptions: \(parsedDates)")
        self.selectedDate = defaultDate

        print("🔢 Date Options Extracted: \(parsedDates)")
    }
